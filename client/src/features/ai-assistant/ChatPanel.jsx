...
function MessageBubble({ role, content }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3`}>
      {!isUser && (
        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#002868] to-[#C8A951] flex items-center justify-center text-[10px] text-white font-bold mr-2 mt-0.5 shrink-0 shadow-sm">
          AI
        </div>
      )}
      <div
        className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${
          isUser
            ? 'bg-[#002868] text-white rounded-tr-sm shadow-sm'
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center gap-3">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#002868] to-[#C8A951] flex items-center justify-center text-2xl shadow-md">
              🔬
            </div>
            <div>
            ...
            <div className="flex flex-col gap-1.5 mt-2">
              {[
                ...
                <button
                  ...
                  className="text-xs text-[#002868] hover:text-[#001f52] bg-white hover:bg-[#002868]/5 border border-[#C8A951]/20 rounded-xl px-3 py-2 transition-colors text-left shadow-sm"
                >
                  {hint}
                </button>
              ))}
          ...
          <>
            ...
            {sending && (
              <div className="flex justify-start mb-3">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#002868] to-[#C8A951] flex items-center justify-center text-[10px] text-white font-bold mr-2 mt-0.5 shrink-0">
                  AI
                </div>
      ...
      <div className="px-6 py-4 bg-white border-t border-[#E4E7EF]">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            className="flex-1 bg-[#F8FAFC] border border-[#E4E7EF] focus:border-[#C8A951]/50 focus:ring-2 focus:ring-[#C8A951]/20 rounded-xl px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 outline-none resize-none transition-all"
            style={{ maxHeight: '120px', overflowY: 'auto' }}
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || sending}
            className="px-4 py-2.5 bg-[#002868] hover:bg-[#001f52] disabled:opacity-40 text-white text-sm font-semibold rounded-xl transition-colors shrink-0 shadow-sm"
          >
            Send
          </button>
        </div>
